  ...
  template: `
    <div class="min-h-screen bg-gray-50 flex">
      ...
      <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0"
             [class.translate-x-0]="isMobileMenuVisible()"
             [class.-translate-x-full]="!isMobileMenuVisible()">
        <!-- Sidebar Header -->
        <div class="flex items-center justify-between h-16 px-6 border-b border-gray-200">
          <div class="flex items-center space-x-3">
            ...
            <h1 class="text-xl font-heading font-bold text-primary-900">StudyBridge</h1>
          </div>
          <button 
        ...
        <div class="p-6 border-b border-gray-200" *ngIf="currentUser()">
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-primary-500 rounded-full flex items-center justify-center">
              <span class="text-white font-semibold text-sm">{{ userInitials() }}</span>
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900 truncate">{{ currentUser()?.displayName }}</p>
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
          ...
          <div class="relative">
            <button 
              (click)="toggleVocabularyMenu()"
              class="flex items-center justify-between w-full px-4 py-3 text-sm font-medium text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <svg class="w-4 h-4 transform transition-transform" 
                   [class.rotate-180]="isVocabularyMenuOpen()"
                   fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>
            <div class="ml-8 mt-2 space-y-1" *ngIf="isVocabularyMenuOpen()">
              <a routerLink="/learning" 
                 class="flex items-center px-4 py-2 text-sm text-gray-600 hover:text-primary-600 hover:bg-primary-50 rounded-md">
                Learning
                <span class="ml-2 px-2 py-0.5 bg-amber-100 text-amber-800 text-xs rounded-full">Coming Soon</span>
              </a>
        ...
        <div class="p-4 border-t border-gray-200">
          <p class="text-xs text-gray-500">StudyBridge v1.0.0</p>
          <p class="text-xs text-gray-400">© 2024 StudyBridge</p>
        </div>
      </aside>
          ...
          <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center space-x-4">
              ...
              <button class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM10.07 2.82l3.47 5.2c.09.13.14.28.14.44v6.78l5.83 3.5c.25.15.41.42.41.72v1.07c0 .47-.38.85-.85.85H4.85c-.47 0-.85-.38-.85-.85v-1.07c0-.3.16-.57.41-.72L10.07 15V8.46c0-.16.05-.31.14-.44l.86-1.29"></path>
                </svg>
              </button>
              ...
              <div class="relative" *ngIf="currentUser()">
                <button 
                  ...
                  <div class="hidden sm:block text-left">
                    <p class="text-sm font-medium text-gray-900">{{ currentUser()?.displayName }}</p>
                    <p class="text-xs text-primary-600">{{ currentUser()?.roles?.[0]?.name || 'Student' }}</p>
                  </div>
